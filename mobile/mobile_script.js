...
updateBoatMap();
setTimeout(function () {
  showBoatSpeech("左右滑动划船，看到岛就点。");
}, 500);
setTimeout(function () {
  showBoatSpeech("继续左右滑动，可以看到 UIUX、其他项目和更多实验。");
}, 3600);
...
(function(){
  var isTouch = window.matchMedia && (window.matchMedia('(hover: none) and (pointer: coarse)').matches || window.matchMedia('(max-width: 820px)').matches);
  if(!isTouch) return;
  var startX=0,startY=0,lastX=0,lastY=0,moved=false;
  window.addEventListener('touchstart',function(e){
    if(e.touches.length!==1) return;
    startX=lastX=e.touches[0].clientX; startY=lastY=e.touches[0].clientY; moved=false;
  },{passive:true});
  window.addEventListener('touchmove',function(e){
    if(e.touches.length!==1) return;
    var x0=e.touches[0].clientX, y0=e.touches[0].clientY;
    var dx=x0-lastX, dy=y0-lastY; lastX=x0; lastY=y0;
    if(typeof currentPage!=='undefined' && currentPage===2 && Math.abs(dx)>Math.abs(dy)){
      moved=true;
      if(typeof boatProgress!=='undefined'){
        boatProgress = Math.max(0, Math.min(1, boatProgress - dx/950));
        if(typeof updateBoatMap==='function') updateBoatMap();
      }
      e.preventDefault();
    }
  },{passive:false});
  window.addEventListener('touchend',function(){
    ...
    if(typeof currentPage!=='undefined' && currentPage===2 && Math.abs(dx)>Math.abs(dy)) return;
    if(Math.abs(dy)>Math.abs(dx)){
      if(dy<0 && typeof nextPage==='function') nextPage();
      if(dy>0 && typeof prevPage==='function') prevPage();
    }
  },{passive:true});
  try{
    var params=new URLSearchParams(location.search); var p=params.get('page');
    if(p!==null && typeof goToPage==='function') setTimeout(function(){goToPage(Number(p)||0)},100);
  }catch(e){}
})();
